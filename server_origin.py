...
class SensorFactory(GstRtspServer.RTSPMediaFactory):
	def __init__(self, **properties): 
		...
		self.launch_string = 'appsrc name=source is-live=true block=true format=GST_FORMAT_TIME caps=video/x-raw,format=BGR,width=1920,height=1080,framerate={}/1 ! videoconvert ! video/x-raw,format=I420 ! x264enc speed-preset=ultrafast tune=zerolatency ! rtph264pay config-interval=1 name=pay0 pt=96'.format(self.fps)
		self.img = None
...
class GstServer(GstRtspServer.RTSPServer): 
	def __init__(self,sub_dir,port, **properties ): 
		...
		basic = GstRtspServer.RTSPAuth.make_basic("admin", "4321")
